...
public class missRona implements Challenge {
    ...
    public void getWinner(Queen[] remainingQueens) {
        ...
        for (int i = 0; i < remainingQueens.length; i++) {
            if (highestScore == getMissRonaScore(remainingQueens[i])) {
                missRonaWinner = remainingQueens[i].getDragName();
                System.out.println();
                System.out.println("The Miss Rona winner(s): " + missRonaWinner);
    ...
    public double getMissRonaScore(Queen ex) {
        return ex.getHumorStat() * ex.getActingStat() * ex.getUniqStat() / 3;
    }
}
