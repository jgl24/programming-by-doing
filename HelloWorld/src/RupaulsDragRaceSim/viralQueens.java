...
public class viralQueens implements Challenge {
    ...
    public void getWinner(Queen[] remainingQueens) {
        ...
        String viralQueenWinner;


        for (int i = 0; i < remainingQueens.length; i++) {
            if (!remainingQueens[i].getIsEliminated()) {

                double queenScore = getViralQueenScore(remainingQueens[i]);
        ...
        for (int i = 0; i < remainingQueens.length; i++) {
            if (highestScore == getViralQueenScore(remainingQueens[i])) {
                viralQueenWinner = remainingQueens[i].getDragName();
                System.out.println();
                System.out.println("The Viral Queen winner(s): " + viralQueenWinner);
    ...
    public double getViralQueenScore(Queen ex) {
        return ex.getHumorStat() * ex.getPersonalityStat() * ex.getDanceStat() / 3;
    }
}
